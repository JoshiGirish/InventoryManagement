/*
    This code is used to create pdf of the purchase invoice when the user clicks on
...
function generatePDF(data){
    ...
    function bodyRows(ppes) {
      ...
      for (var j = 0; j < rowCount; j++) {
        ...
        body.push({
          ...
          rate: String(Math.round((ppes[j].price * 100) / 100).toFixed(2)),
  ...
  margins = {
    ...
    str = String(data.po.po);
    doc.setFontSize(15);
    doc.text(str, pageWidth-margins.right, atLine,'right');
    atLine += lineHeight+2;
    
    str = "Date: "+ moment(data.po.date).format('MMMM Do YYYY');
    doc.setFontSize(9);
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+2;
    
    str = "Shipping Date: "+ moment(data.po.date).format('MMMM Do YYYY');
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+2;
    ...
    ///////////////MAIN TABLE///////////////////
      ...
      let head = headRows()
      let body = bodyRows(data.po.ppes)
    ...
    str = String(Math.round((data.po.subtotal * 100) / 100).toFixed(2));
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+3;
    
    str = "TAX AMOUNT :";
    doc.text(str, pageWidth-margins.right-40, atLine+2,'right');
    str = String(Math.round((data.po.taxtotal * 100) / 100).toFixed(2));
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+3;
    
    str = "ROUNDED OFF :";
    doc.text(str, pageWidth-margins.right-40, atLine+2,'right');
    str = String(Math.round((data.po.ordertotal * 100) / 100).toFixed(2));
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+3;
    ...
    str = toTitleCase(numWords(Math.round(data.po.ordertotal)));
    doc.text(str, pageWidth-margins.right, atLine+2,'right');
    atLine += lineHeight+3;
